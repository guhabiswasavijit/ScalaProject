...
object MutualInformation extends App {
  ...
  val conf = new SparkConf().setAppName("lingSpam").setMaster("local")
  val sc = new SparkContext(conf)

  val (pPresentGivenSpam, nSpam) = wordFractionInFiles(sc)("C:\\SampleData\\ling-spam\\spam\\*")
  val pAbsentGivenSpam = pPresentGivenSpam.mapValues { 1.0 - _ }
  val (pPresentGivenHam, nHam) = wordFractionInFiles(sc)("C:\\SampleData\\ling-spam\\ham\\*")
